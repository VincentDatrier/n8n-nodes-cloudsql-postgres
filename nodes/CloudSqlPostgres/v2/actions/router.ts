...
import { addExecutionHints } from '../../shared/utilities';
import { configureCloudSqlPostgres } from '../../transport';
import type { CloudSqlNodeCredentials, PostgresNodeOptions } from '../helpers/interfaces';
...
export async function router(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	const credentials = await this.getCredentials<CloudSqlNodeCredentials>('cloudSqlPostgres');
	const options = this.getNodeParameter('options', 0, {}) as PostgresNodeOptions;
	const node = this.getNode();
	options.nodeVersion = node.typeVersion;
	options.operation = operation;

	const { db, pgp } = await configureCloudSqlPostgres.call(this, credentials, options);
